...
class Agent:
    def __init__(self, x, y):
        self.x = x
        self.y = y
        self.energy = 10  # Valeur de départ, modifiable selon ton modèle

    def perceive(self, grid):
        # Retourner une liste des 8 cellules autour de l'agent
        neighbors = []
    ...
    def choose_move(self, neighbors):
        # Priorité : nourriture > phéromone > vide
        food_cells = []
        pheromone_cells = []
        empty_cells = []
        ...
        if food_cells:
            ...
            return (self.x, self.y)  # Rester sur place si bloqué

    def move(self, new_position):
        self.x, self.y = new_position

    def deposit_pheromone(self, grid):
        # On pourrait ajouter une quantité de phéromone ici
